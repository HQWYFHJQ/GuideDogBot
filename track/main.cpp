...
//机器狗控制参数
int gait_type=3;//运动是3，停止}是4
float step_height=0.04;
float stand_height=0.3;
float v_des[3]={0.2,0,0};
...
void imageProcess(Mat &image){
    ...
    if(mythread.mode==divergeright){
        if(mythread.diverge2TransientProcess==1){
            goalAverage=200;
            average=getAverage(frame);
            gait_type=3;step_height=0.03;stand_height=0.3;
            rpy_des[0]=0;rpy_des[1]=0;rpy_des[2]=0;
            v_des[0]=0.2;//前进
        ...
        goalAverage=180;
        average=getAverage(frame);
        gait_type=3;step_height=0.04;stand_height=0.3;//gait_type=3;step_height=0.09;stand_height=0.3
        rpy_des[0]=0;rpy_des[1]=0;rpy_des[2]=0;
        v_des[0]=0.2;
        v_des[2]=-0.15;
        }
